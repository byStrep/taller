...
function loadData() {
  try {
    ...
    return { clientes: [], tecnicos: [], vehiculos: [] };
  }
}
...
const Label = ({ children }) => (
  <div style={{ fontSize: 12, color: "#9bb0d3" }}>{children}</div>
);
const Row = ({ children, cols = 2, gap = 12, style }) => (
  <div
    style={{
      display: "grid",
...
export default function App() {
            ...
            <Box style={{ display: "grid", gap: 12 }}>
              <Row cols={2}>
                <div>
                  ...
                  <div style={{ fontWeight: 600 }}>{v.matricula || "—"}</div>
                </div>
                <div>
                  <Label>Modelo</Label>
                ...
                <div>
                  <Label>Entrada</Label>
                  <div style={{ fontWeight: 600 }}>{v.entrada}</div>
                </div>
                <div>
                  <Label>Estado</Label>
                  <div
              ...
              <div>
                ...
                {v.diagnosticos.length === 0 && (
                  <div style={{ fontSize: 14, color: "#9bb0d3" }}>
                    Aún no hay entradas.
                  </div>
                )}
                {v.diagnosticos
  ...
  const list = data.vehiculos.filter((v) => {
    ...
    if (!q) return true;
    return [v.matricula, v.marca, v.modelo].some((x) =>
  ...
  function upsertVehicle(v) {
    setData((prev) => {
      ...
      const vehiculos = exists
        ? prev.vehiculos.map((x) => (x.id === v.id ? v : x))
        : [v, ...prev.vehiculos];
  ...
  function upsertClient(c) {
    setData((prev) => {
      ...
      const clientes = exists
        ? prev.clientes.map((x) => (x.id === c.id ? c : x))
        : [c, ...prev.clientes];
  ...
  function addDiagnosis(vehicleId) {
    ...
    setData((prev) => ({
      ...prev,
      vehiculos: prev.vehiculos.map((v) =>
        v.id === vehicleId
          ? { ...v, diagnosticos: [...v.diagnosticos, newDiag(autor)] }
          : v
      ),
    }));
  }
    ...
    setData((prev) => ({
      ...
      vehiculos: prev.vehiculos.map((v) =>
        v.id === vehicleId
          ? {
              ...v,
              diagnosticos: v.diagnosticos.map((d) =>
                d.id === diagId ? { ...d, ...patch } : d
              ),
            }
          : v
      ),
  ...
  React.useEffect(() => {
    if (data.clientes.length === 0 && data.vehiculos.length === 0) {
      ...
      const v = {
        ...
        matricula: "1234 ABC",
        marca: "Hyundai",
        modelo: "i20 1.0 T-GDI",
        anio: "2024",
      };
      v.diagnosticos.push({
        ...newDiag("Aaron (demo)"),
        descripcion: "Revisión 30.000 km. Ruidos en eje trasero.",
        acciones: "Inspección y apriete.",
        presupuesto: "150€ aprox",
      });
      setData({
        clientes: [c],
        vehiculos: [v],
        tecnicos: [{ ...newTech(), nombre: "Aaron (demo)", usuario: "aaron", password: "demo" }],
      });
    }
  ...
  return (
    ...
    >
      <div style={{ maxWidth: 1100, margin: "24px auto", padding: 16 }}>
        ...
        <Row cols={3} gap={8}>
          <Input
            placeholder="Buscar matrícula / modelo"
            value={search}
          ...
          <Button
            onClick={() => {
              const c = newClient();
              upsertClient(c);
              alert("Cliente creado (rellena en la ficha)");
            }}
          >
            Nuevo cliente
          </Button>
        ...
        >
          {list.map((v) => (
            ...
            >
              ...
              <div style={{ color: "#9bb0d3", fontSize: 14, marginTop: 4 }}>
        ...
        {vehiculoSel && (
          <Box style={{ marginTop: 16 }}>
            ...
            >
              <h3 style={{ margin: 0 }}>Ficha de vehículo</h3>
              <Button onClick={() => setSelectedId(null)}>Cerrar ficha</Button>
            </div>

            <Row cols={4} gap={8} style={{ marginTop: 8 }}>
              <div style={{ gridColumn: "span 2" }}>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    const ex = data.vehiculos.find(
                      (x) =>
                        normalizeMatricula(x.matricula) ===
                        normalizeMatricula(formatted)
                    );
                ...
                {vehiculoSel.matricula &&
                  !isMatriculaValida(vehiculoSel.matricula) && (
                    <div style={{ color: "#ff9b9b", fontSize: 12, marginTop: 4 }}>
                      Formato no válido
                    </div>
                  )}
              </div>
              ...
              <div>
                <Label>Marca</Label>
                <Input
                  value={vehiculoSel.marca}
                  onChange={(e) =>
                    upsertVehicle({ ...vehiculoSel, marca: e.target.value })
                  }
                />
              </div>
              <div>
                <Label>Modelo</Label>
                <Input
                  value={vehiculoSel.modelo}
                  onChange={(e) =>
                    upsertVehicle({ ...vehiculoSel, modelo: e.target.value })
                  }
                />
              </div>
              <div>
                <Label>Año</Label>
                <Input
                  value={vehiculoSel.anio}
                  onChange={(e) =>
                    upsertVehicle({ ...vehiculoSel, anio: e.target.value })
                  }
                />
              </div>
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    upsertVehicle({ ...vehiculoSel, estado: e.target.value })
                  }
                  style={{
                    padding: "8px 10px",
                    borderRadius: 8,
              ...
              <div>
                ...
                <Input
                  value={vehiculoSel.entrada}
                  onChange={(e) =>
                    upsertVehicle({ ...vehiculoSel, entrada: e.target.value })
                  }
                />
              </div>
            </Row>
            ...
            <Row cols={2} gap={12} style={{ marginTop: 12 }}>
              <Box>
                ...
                <select
                  ...
                  onChange={(e) =>
                    upsertVehicle({ ...vehiculoSel, clienteId: e.target.value })
                  }
                  style={{
                    padding: "8px 10px",
                    borderRadius: 8,
                ...
                {!vehiculoSel.clienteId && (
                  <div style={{ marginTop: 8 }}>
                    <h5 style={{ marginTop: 0 }}>Crear cliente rápido</h5>
                    <QuickClient
                      onCreate={(c) => {
                        upsertClient(c);
                        upsertVehicle({ ...vehiculoSel, clienteId: c.id });
                      }}
                    />
                  </div>
                )}
              ...
              <Box>
                <h4 style={{ marginTop: 0 }}>Enlace público</h4>
                <Input
                  readOnly
                  value={`${location.origin}${location.pathname}?view=${vehiculoSel.tokenPublico}`}
                />
                <div style={{ marginTop: 8 }}>
                  <Button onClick={() => copyPublicLink(vehiculoSel)}>
                    Copiar enlace
                  </Button>
                </div>
              </Box>
            ...
            <div style={{ marginTop: 12 }}>
              ...
              >
                <h4 style={{ margin: 0 }}>Diagnósticos</h4>
                <Button onClick={() => addDiagnosis(vehiculoSel.id)}>
                  Añadir entrada
                </Button>
              </div>

              {vehiculoSel.diagnosticos.length === 0 && (
                <div style={{ fontSize: 14, color: "#9bb0d3" }}>
                  Sin entradas aún.
                </div>
              )}
                ...
                {vehiculoSel.diagnosticos.map((d) => (
                  <Box key={d.id}>
                    <Row cols={4} gap={8}>
                      <Input
                        readOnly
                        value={new Date(d.fecha).toLocaleString()}
                      />
                      <Input
                        value={d.autor}
                        onChange={(e) =>
                          updateDiag(vehiculoSel.id, d.id, {
                            ...
                            setData((prev) => ({
                              ...
                              vehiculos: prev.vehiculos.map((v) =>
                                v.id === vehiculoSel.id
                                  ? {
                                      ...v,
                                      diagnosticos: v.diagnosticos.filter(
                                        (x) => x.id !== d.id
                                      ),
                                    }
                                  : v
                    ...
                    <Textarea
                      ...
                      placeholder="Acciones realizadas / pendientes"
                    />
                  </Box>
                ))}
              </div>
